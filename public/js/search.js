    ...
    fetch(`/api/search/${searchText}`)
        ...
        .then(results => {
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                const eventHtml = 
                    `<div onclick="showEvent(${eventObj.id})" class="bg-gray-200 p-4 rounded cursor-pointer">
                        <h2>${eventObj.event_name}</h2>
                        <p id="desc">${eventObj.event_desc}</p>
                        <p>Start: ${startDate} End: ${endDate}</p>
                        <p>Your event</p>
                    </div>`
                const element= document.getElementById("searchContainer").innerHTML += eventHtml;
            }
